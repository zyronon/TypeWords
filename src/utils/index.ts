    ...
    if (val) {
        try {
            ...
            if (version === SAVE_DICT_KEY.version) {
                ...
                defaultState.article.bookList =
                    defaultState.article.bookList.map((v) => {
                        return getDefaultDict(
                            checkRiskKey(getDefaultDict(), v),
                        );
                    });
                defaultState.word.bookList = defaultState.word.bookList.map(
                    (v) => {
                        return getDefaultDict(
                            checkRiskKey(getDefaultDict(), v),
                        );
...
export function checkAndUpgradeSaveSetting(val: any) {
    ...
    if (val) {
        try {
            ...
            if (typeof state !== "object") return defaultState;
            state.load = false;
            state.language = sanitizeSourceLanguage(state.language);
            state.targetLanguage = sanitizeTargetLanguage(state.targetLanguage);
            let version = Number(data.version);
            if (version === SAVE_SETTING_KEY.version) {
                ...
                checkRiskKey(defaultState, state);
                defaultState.language = sanitizeSourceLanguage(
                    defaultState.language,
                );
                defaultState.targetLanguage = sanitizeTargetLanguage(
                    defaultState.targetLanguage,
                );
                return defaultState;
            } else {
                if (version === 13) {
...
export function shakeCommonDict(n: BaseState): BaseState {
    ...
    data.word.bookList.map((v: Dict) => {
        if (
            !v.custom &&
            ![DictId.wordKnown, DictId.wordWrong, DictId.wordCollect].includes(
                v.id,
            )
        )
            v.words = [];
...
export async function _getDictDataByUrl(
    ...
    if (!payload) {
        console.warn("[dict] Failed to load dict resource", {
            url: val.url,
            candidates,
            error: lastError,
        });
