...
describe('cross-platform practice word typing', () => {
  ...
  it('does not count a correct direct spelling answer in identify mode as wrong', () => {
    const { typing, events, getShowWordResult } = createTyping(false, WordPracticeType.Identify)
  ...
  it('counts an incorrect direct spelling answer in identify mode as wrong', () => {
    const { typing, events } = createTyping(false, WordPracticeType.Identify)

    for (const key of ['A', 'X', 'C']) typing.typeCharacter({ key })

    expect(typing.isWordCorrect.value).toBe(false)
    expect(events.wrong).toHaveBeenCalledOnce()
  })
})
