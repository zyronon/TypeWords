...
async function loadPractice() {
  ...
  loading.value = true
  try {
    const { dict, items } = await sentenceInit.loadItemsByDictId(dictId.value)
    if (!dict.id) {
      router.push('/words')
      Toast.warning('词书不存在')
      return
        ...
        <div class="sentence-topbar">
          ...
          <div class="mode-switch" role="tablist" aria-label="句子练习模式">
            <div
              ...
              @keydown.enter.prevent="setMode(option.value)"
              @keydown.space.prevent="setMode(option.value)"
            >
              {{ option.label }}
            </div>
          </div>
    ...
    <template v-slot:footer>
      <div class="sentence-footer">
        <div
          ...
          @keydown.enter.prevent="settingStore.showPanel = !settingStore.showPanel"
          @keydown.space.prevent="settingStore.showPanel = !settingStore.showPanel"
        >
