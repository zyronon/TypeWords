...
3. **显隐二分**：
   - `WordPracticeMode.Free` → 继续读 `settingStore.dictation` / `settingStore.translate`
   - 其他模式 → 读 `sessionDisplay` + `displayOverride`；**禁止**用 settingStore 两字段做显隐
...
阶段变化 / 缓存恢复 → resolvePhase() → applyPhase()
  ├─ Free：display.source = 'settingStore'，不写入 sessionDisplay
  └─ 其他：sessionDisplay 来自 Registry；用户 Toggle 只写 displayOverride；进下一阶段清空 override

刷新恢复：restoreSession() 唯一入口；结构化模式 snapshot 存 sessionDisplay + override，不存 dictation/translate
```
...
| Review | 自测旧→听写旧→默写旧 | |
| IdentifyOnly / DictationOnly / ListenOnly | 各 2 阶段 | |
...
6. Shepherd Tour、Umami 埋点、Conflict/Collect 弹窗 `setTimeout` 时序——与练习核心无关。
7. `if (import.meta.client) {}` 空块、多处 `console.log` 调试残留。
8. `getDefaultPracticeData` 用 `Object.assign(origin, ...)` 会 mutate 传入的 origin，易出隐蔽 bug。

**设计缺陷**

9. **`usePracticeStore` 单词/文章共用**：[`practice.ts`](../packages/core/src/stores/practice.ts) 被单词页和 [`practice-articles`](../apps/nuxt/app/pages/(articles)/practice-articles/[id].vue) 共用，切换练习类型时存在状态残留风险。
10. **错词处理逻辑三处重复**：`next()` 内、`onWordMarkPickComplete`、`WordMarkPickList` 完成回调，结构相同但复制粘贴。
...
5. 例句列表 UI（990-1025 行）与输入逻辑缠在一起；跟写完成后 `currentPracticeSentenceIndex++` 从 -1 变 0 的隐式状态切换难懂。
6. `SENTENCE_PLAY_SHORTCUT_KEYS` 硬编码 9 个快捷键与 sentences 数组索引绑定。
7. `sentenceVolumeIconsRefs: any` 无类型。
...
interface PracticeDisplayPolicy {
  /** 自由模式：读 settingStore；结构化模式：读 sessionDisplay（Registry 推导） */
...
#### 自由模式（`WordPracticeMode.Free`）在 Registry 中的特殊地位

自由模式是**最简单但规则不同**的一类，必须在 Registry 里单独建模，不能套用 System 的子集。

**现状行为梳理**（[`practice-words/[id].vue`](../apps/nuxt/app/pages/(words)/practice-words/[id].vue)）：

| 维度 | 自由模式 | 系统模式等 |
|------|----------|------------|
| 阶段链 | 仅 `FollowWriteNewWord → Complete`（[`env.ts`](../packages/core/src/config/env.ts)） | 多阶段流转 |
| 显隐 | **`watchPracticeType` 直接 return**，不自动改 dictation/translate；用户通过 Footer 图标/快捷键手动切换 | 随阶段自动设定 |
| 词内导航 | 仅 `index++`，**无 `wordLoop`（7 词分组）**，无 Spell 子相位 | 跟写↔拼写循环 |
| 列表结束 | 有错词 → shuffle 错词重练；无错词 → `complete()` | 按 stage 推进下一阶段 |
| 错词循环 | `isTypingWrongWord=true`，强制 `wordPracticeType=FollowWrite`，但**仍不覆盖**用户 dictation/translate | 同上结构，但会进 stage 机 |
| Footer | 单进度条、无「跳过本阶段」、状态文案「自由练习」 | 多阶段进度条 + skipStep |
| 面板 | 有「下一组」仅 Free 时显示（vscode-web/nuxt 略有差异） | GroupList + continueStudy |

**显隐数据源规则（产品约束，v2 必须遵守）**

| 模式 | 显隐数据源 | 用户 Toggle 行为 | 阶段/模式切换时 |
|------|------------|------------------|-----------------|
| **自由模式** | 继续用 `settingStore.dictation` / `settingStore.translate` | 直接改 settingStore（与现网一致） | 不自动覆盖；刷新后随 setting 持久化恢复 |
| **其他所有模式** | **不用** settingStore 两字段；由 Registry → `sessionDisplay` | 可临时改 `sessionDisplayOverride`（仅当前相位有效） | `applyPhase()` **强制**按阶段定义重置，覆盖用户临时操作 |

> settingStore 的 `dictation`/`translate` 在结构化模式下**退化为全局默认/历史偏好**，不再参与练习页显隐计算；避免与阶段机打架，也是刷新错乱的根本治理方向。

**Registry 定义示例**：

```ts
const FREE_NORMAL: PracticePhaseDefinition = {
  key: { mode: Free, stage: FollowWriteNewWord, practiceType: FollowWrite },
  display: {
    source: 'settingStore',  // ★ 读 settingStore.dictation / translate
    inputMode: 'typing',
    autoNextWord: true,
  },
  wordAdvance: { type: 'increment' },
  stageAdvance: { complete: true, wordsFrom: 'current' },
}

const FREE_WRONG_REVIEW: PracticePhaseDefinition = {
  key: { mode: Free, stage: FollowWriteNewWord, practiceType: FollowWrite, isTypingWrongWord: true },
  display: { source: 'settingStore', inputMode: 'typing' },
  wordAdvance: { type: 'increment' },
  stageAdvance: { complete: true, wordsFrom: 'wrongWords', shuffle: true, forceWrongWordMode: true },
}

// 结构化模式示例：自测旧词
const SYSTEM_IDENTIFY_REVIEW: PracticePhaseDefinition = {
  key: { mode: System, stage: IdentifyReview, practiceType: Identify },
  display: {
    source: 'phase',
    wordMask: 'none',
    showPhonetic: false,
  ...
  wordAdvance: { type: 'increment' },
  stageAdvance: { nextStage: ListenReview, wordsFrom: 'taskReview', shuffle: true },
}
```
...
```ts
function applyPhase(ctx, phase) {
  sessionUI.practiceType = phase.key.practiceType

  if (phase.display.source === 'settingStore') {
    // 自由模式：不碰 settingStore；TypeWord 直接读 settingStore
    sessionDisplayOverride = null
    return
  }

  // 结构化模式：从 Registry 推导 sessionDisplay，清空临时覆盖
  sessionDisplay = deriveDisplay(phase.display)
  sessionDisplayOverride = null   // ★ 进入新阶段/模式 → 强制重置，忽略用户刚才的 Toggle
}
```
...
function toggleDictation() {
  ...
  // 只改 override，不写 settingStore
  sessionDisplayOverride = {
    ...sessionDisplayOverride,
    wordMask: sessionDisplayOverride?.wordMask === 'none' ? 'underscore' : 'none',
  }
}
```

**`usePracticeDisplayPolicy` 读取规则**：

```ts
const effective = computed(() => {
  const base = policy.source === 'settingStore'
    ? deriveFromSettingStore(settingStore.dictation, settingStore.translate)
    : mergeDisplay(sessionDisplay, sessionDisplayOverride)

  return applyLocalReveal(base, { showFullWord, showWordResult })
})
```

- 自由模式：逻辑与现 TypeWord 一致，数据源仍是 `settingStore`
- 结构化模式：TypeWord / Footer **禁止**读 `settingStore.dictation/translate` 做显隐

```mermaid
flowchart TB
  subgraph structured [系统/听写/自测等]
    stageChange["阶段/模式变化"] --> applyPhase["applyPhase"]
    applyPhase --> sessionDisplay["sessionDisplay 系统设定"]
    applyPhase --> clearOverride["清空 override"]
    userToggle["用户临时 Toggle"] --> override["sessionDisplayOverride"]
    sessionDisplay --> effective["effective = merge + localReveal"]
    override --> effective
  end
  subgraph freeMode [自由模式]
    userToggleFree["用户 Toggle"] --> settingStore["settingStore.dictation/translate"]
    settingStore --> effectiveFree["effective"]
  end
```

**导航器 `next()` 自由模式分支**（替代现有独立 if 块）：

```ts
// resolvePhase 返回 FREE_NORMAL 或 FREE_WRONG_REVIEW
// navigator 统一：
if (atListEnd) runWrongWordRetryOr(phase.stageAdvance)
else runWordAdvance(phase.wordAdvance)
// 自由模式不会进入 runStageAdvance 多阶段链
```

**验收补充（自由模式）**：
- 手动关闭翻译/默写后刷新，显隐保持不变
- 练完全部词有错词 → 错词 shuffle 重练 → 再 complete
- 无 wordLoop、无 skipStep、无阶段自动改显隐
...
更关键的是 [`initData` 缓存恢复分支](../apps/nuxt/app/pages/(words)/practice-words/[id].vue)（`init=true` 且有完整缓存）：
...
interface PracticeSessionSnapshot {
  ...
  // 注意：自由模式不写 dictation/translate，继续走 settingStore 全局持久化
}
```
...
| 场景 | dictation / translate 来源 | 刷新恢复 |
|------|---------------------------|----------|
| 自由模式 | `settingStore`（沿用现网，全局 idb 持久化） | 读 settingStore 即可 |
| 结构化模式 | `sessionDisplay` + 可选 `displayOverride` | snapshot 存 display/override；无 snapshot 时 `applyPhase(stage)` 强制推导 |

| 操作 | 自由模式 | 结构化模式 |
|------|----------|------------|
| Footer Toggle 默写/翻译 | 改 `settingStore` | 改 `displayOverride`，**不改** settingStore |
| 进入下一阶段 | 不自动改 settingStore | `applyPhase` 重置 `sessionDisplay`，清空 override |
| TypeWord 显隐 | 读 settingStore | 读 `effective`（sessionDisplay + override） |

这样「自测只显示单词」由 Registry 写入 `sessionDisplay`，刷新后 `applyPhase` 或 snapshot 精确还原，**与 settingStore 里残留的旧值无关**。
...
| 自由模式默写 Toggle | **不改变** `canSeeSentences`（仍 true） | 用户以为例句「藏起来了」，却仍可能自动播 |
| 结构化模式 | `canSeeSentences` 不看 `sessionDisplay` | 刷新/阶段切换后 UI 与自动播放可能不一致 |
...
- **Registry** 各相位定义 `showSentences` / `showSentenceTranslation`；自由模式由 `settingStore.dictation/translate` 推导进 `effective`（与显隐二分规则一致）
...
flowchart LR
  ...
  settingStoreFree["settingStore 仅 Free"] --> effective
...
- 自由模式开启默写隐藏例句后：不自动播；关闭默写例句可见后：恢复自动播
- 临时「显示单词」后例句露出：与 UI 同步，可按规则自动播

#### `watchStage` / `watchPracticeType` 的处理

**整段删除**。结构化模式的显隐不再经由 `watchPracticeType` 写 settingStore；改由 `applyPhase` → `sessionDisplay`。自由模式本来就会 `watchPracticeType` return，保持不变（读 settingStore）。
...
- **自由模式**：手动切换默写/翻译后刷新 10 次，显隐与 settingStore 一致
- **系统模式自测阶段**：临时打开翻译可见，进入下一阶段后自动收回；刷新后仍按 stage 显示（无例句）；**且不自动播例句**
...
4. **结构化模式 Footer Toggle 不得写 settingStore**：只写 `displayOverride`
5. **自由模式是唯一读 settingStore 显隐的模式**：TypeWord v2 用 `policy.source` 分支
